<script lang="ts">
  ...
  let toggleCall = data.toggleCall;
  ...
  $: data, $activeCallId;
  $: cantToggleCall = !!data.activeCall && data.selected !== data.activeCall;
...
{#if localStorage.getItem("currentUser")}
  <div class="h-screen w-full">
    {#if !!$activeCallId && !!data.activeCallFriend}
      <div class="h-2/6 p-2 w-full relative top-0 z-10">
        <CallScreen user1={data.me} user2={data.activeCallFriend} />
      </div>
    {/if}
    <div
      id="chat-screen"
      class={`flex flex-col justify-end ${
        !!$activeCallId ? "h-4/6" : "h-full"
      } w-full relative`}
    >
      ...
      <!--Header on Selected Friend-->
      <div
        id="header"
        class="prose flex flex-row items-center gap-x-4 w-[inherit] max-w-[inherit] py-4 px-4 backdrop-blur absolute shadow-lg top-0 bg-base-200/90"
      >
        <label
          for="friend-list"
        ...
        <strong class="text-lg">{data.selectedFriend.name}</strong>
        <div class="flex rounded-lg bg-base-300 h-fit translate-y-[1px]">
          <code>{data.selectedFriend.id}</code>
        </div>
        <div class="flex-1" />
        <div class="btn-group">
          <button
            disabled={cantToggleCall}
            class="drawer-button btn btn-accent p-3"
            on:click={async () => {
              console.log(`Calling ${data.selectedFriend?.id}...`);
              data.selectedFriend && toggleCall(data.selected);
              await invalidateAll();
            }}
          >
            <Icon
