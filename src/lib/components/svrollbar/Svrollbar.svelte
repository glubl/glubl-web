<script>
  ...
  $: shouldMargin = marginTop != 0 || marginBottom != 0 || marginLeft != 0 || marginRight != 0

  $: wholeHeight = viewport?.scrollHeight ?? 0
  $: scrollTop = viewport?.scrollTop ?? 0
  $: trackHeight = viewport?.clientHeight ?? 0 - (marginTop + marginBottom)
  $: thumbHeight = wholeHeight > 0 ? (viewport?.clientHeight / wholeHeight) * trackHeight : 0
...
{#if visible}
  <div
    ...
    style={`height: ${trackHeight}px;` + (shouldMargin ? `margin: ${marginTop}px ${marginRight}px ${marginBottom}px ${marginLeft}px` : "")}>
    <div
      bind:this={vTrack}
      class="v-track"
